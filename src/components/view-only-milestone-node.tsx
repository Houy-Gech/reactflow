...
interface ViewOnlyMilestoneNodeProps {
    data: MilestoneData
}

export function ViewOnlyMilestoneNode({ data }: ViewOnlyMilestoneNodeProps) {
    ...
    return (
        <div className={`${getStatusColor(data.status)} text-white rounded-lg p-3 min-w-[200px] shadow-lg border-2 cursor-default`}>
            <Handle type="target" position={Position.Left} className="w-3 h-3" style={{ pointerEvents: 'none' }} />
            ...
            <Handle type="source" position={Position.Right} className="w-3 h-3" style={{ pointerEvents: 'none' }} />
        </div>
    )
}
